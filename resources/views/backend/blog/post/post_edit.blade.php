		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			...
			<div class="box-body">
             ...
             <input type="hidden" name="old_id" value="{{ $blogpost->id }}" >
...
<div class="row"> <!-- start 2nd row  -->
	 ...
	 @enderror
			...
			<div class="controls">
				<input type="text" name="post_title_others" value="{{ $blogpost->post_title_others }}" class="form-control" required="">
...
<div class="controls">
<input type="text" name="post_tag_en" value="{{ $blogpost->post_tag_en }}" data-role="tagsinput" placeholder="add tag" /> 
@error('post_tag_en') 
<span class="text-danger">{{ $message }}</span>
@enderror
</div>
...
<div class="controls">
<input type="text" name="post_tag_others" value="{{ $blogpost->post_tag_others }}" data-role="tagsinput" placeholder="add tag" /> 
...
<div class="row"> <!-- start 6th row  -->
	...
	<div class="controls">
		<select name="category_id" class="form-control" required="" >
         ...
         <option value="{{ $category->id }}" <?php if ($category->id == $blogpost->category_id ) {
                     echo "selected"; } ?>  >{{ $category->blog_category_name_en }}</option>	
			@endforeach
		</select>
		@error('category_id') 
	 <span class="text-danger">{{ $message }}</span>
...
<div class="row"> <!-- start 8th row  -->
	    ...
	    <div class="form-group">
			...
			<div class="controls">
	<textarea id="editor1" name="post_details_en" rows="10" cols="80" required="" >{{ $blogpost->post_details_en }}</textarea>  
	 		 </div>
		</div>
	     ...
	     <div class="form-group">
			...
			<div class="controls">
	<textarea id="editor2" name="post_details_others" rows="10" cols="80">{{ $blogpost->post_details_others }}</textarea>       
	 		 </div>
		</div>
