...
class SliderController extends Controller
{
    ...
    public function SliderStore(Request $request){
    	...
    	Image::make($image)->resize(1903,633)->save('backend/upload/slider/'.$name_gen);
    	$save_url = 'backend/upload/slider/'.$name_gen;

        $image_back = $request->file('slider_img_back');
    	$name_gen_back = hexdec(uniqid()).'.'.$image_back->getClientOriginalExtension();
    	Image::make($image_back)->resize(1903,633)->save('backend/upload/slider/'.$name_gen_back);
    ...
    public function SliderUpdate(Request $request){
            ...
            $image = $request->file('slider_img');
            $name_gen = hexdec(uniqid()).'.'.$image->getClientOriginalExtension();
            Image::make($image)->resize(1903,633)->save('upload/slider/'.$name_gen);
